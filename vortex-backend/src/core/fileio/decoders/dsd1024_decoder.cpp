...
DSD1024Decoder::DSD1024Decoder() : initialized_(false) {}

DSD1024Decoder::~DSD1024Decoder() {
    shutdown();
}

bool DSD1024Decoder::initialize() {
    if (initialized_) {
        return true;
    }

    initialized_ = true;
    Logger::info("DSD1024 decoder initialized successfully");
    return true;
}

void DSD1024Decoder::shutdown() {
    if (!initialized_) {
        return;
    }

    initialized_ = false;
    Logger::info("DSD1024 decoder shutdown");
}

bool DSD1024Decoder::canDecode(const std::string& filePath) const {
    if (!initialized_) {
        return false;
    }
    ...
    return isDSD1024Format(header.data(), bytesRead);
}

std::optional<AudioData> DSD1024Decoder::decode(const std::string& filePath) {
    if (!initialized_) {
        Logger::error("DSD1024 decoder not initialized");
        return std::nullopt;
    }

    std::ifstream file(filePath, std::ios::binary);
    if (!file.is_open()) {
        Logger::error("Cannot open DSD1024 file: {}", filePath);
        return std::nullopt;
    }

    try {
        Logger::info("Decoding DSD1024 file: {}", filePath);

        // Parse DSD file to determine format and properties
        DSDFileInfo fileInfo;
        if (!parseDSDFile(file, fileInfo)) {
            Logger::error("Failed to parse DSD1024 file: {}", filePath);
            return std::nullopt;
        }

        // Verify this is DSD1024 (45.1584 MHz)
        if (fileInfo.sampleRate != 45158400.0) {
            Logger::error("File is not DSD1024 (sample rate: {} Hz)", fileInfo.sampleRate);
            return std::nullopt;
        }
        ...
        audioData.format = AudioFormat::DSD1024;

        // DSD1024 has extremely high sample rate, so we need significant downsampling
        // for practical PCM output. We'll use a downsampling ratio of 1024:1
        // to get a standard high-quality sample rate.
        const int downsampleRatio = 1024;
        const double targetSampleRate = fileInfo.sampleRate / downsampleRatio;

        Logger::debug("DSD1024 downsample ratio: {} ({} Hz -> {} Hz)",
        ...
        if (!decodeDSDToPCM(file, fileInfo, pcmData, totalPCMSamples, downsampleRatio)) {
            Logger::error("Failed to decode DSD1024 to PCM");
            return std::nullopt;
        }

        Logger::info("DSD1024 decoded successfully: {} PCM samples, {} channels, {:.2f} seconds",
        ...
        Logger::error("Exception during DSD1024 decoding: {}", e.what());
        return std::nullopt;
    }
}

bool DSD1024Decoder::extractMetadata(const std::string& filePath, AudioMetadata& metadata) {
    if (!initialized_) {
        return false;
    }
    ...
    try {
        ...
        metadata.format = AudioFormat::DSD1024;
        metadata.codec = "DSD1024";
        ...
        // Calculate bitrate (DSD1024 = 45.1584 MHz per channel)
        ...
        Logger::error("Exception during DSD1024 metadata extraction: {}", e.what());
        return false;
    }
}

bool DSD1024Decoder::isDSD1024Format(const uint8_t* data, size_t size) const {
    if (size < 4) {
        return false;
    }
...
bool DSD1024Decoder::parseDSDFile(std::ifstream& file, DSDFileInfo& fileInfo) {
...
bool DSD1024Decoder::parseDSDIFFFile(std::ifstream& file, DSDFileInfo& fileInfo) {
...
bool DSD1024Decoder::parseDSFFile(std::ifstream& file, DSDFileInfo& fileInfo) {
...
bool DSD1024Decoder::decodeDSDToPCM(std::ifstream& file, const DSDFileInfo& fileInfo,
                                     float* pcmData, uint64_t totalSamples, int downsampleRatio) {
    // Seek to audio data
    file.seekg(fileInfo.dataOffset);

    // For DSD1024, use multi-stage filtering for high-quality downsampling
    // First stage: accumulate DSD samples, second stage: apply digital filter
    ...
    const size_t chunkSize = 262144;  // 256KB chunks for large DSD1024 files
    std::vector<uint8_t> dsdData(chunkSize);
    uint64_t samplesProcessed = 0;
    uint64_t pcmIndex = 0;

    // Output buffer for filtering
    std::vector<float> filteredOutput(downsampleRatio * fileInfo.channels);
                ...
                if (samplesProcessed % downsampleRatio == 0 && pcmIndex < totalSamples) {
                    ...
                    for (uint16_t channel = 0; channel < fileInfo.channels; ++channel) {
                        if (pcmIndex < totalSamples) {
                            ...
                            // Normalize and apply gain reduction for DSD1024
                            filteredValue /= downsampleRatio;
                            filteredValue *= 0.707f;  // -3dB gain reduction
    ...
    if (pcmIndex < totalSamples) {
        for (uint16_t channel = 0; channel < fileInfo.channels; ++channel) {
            if (pcmIndex < totalSamples) {
                ...
                filteredValue *= 0.707f;  // Apply gain reduction
    ...
    Logger::debug("DSD1024 to PCM conversion: {} DSD samples -> {} PCM samples ({}x downsampling)",
                 samplesProcessed, pcmIndex, samplesProcessed / std::max(pcmIndex, 1ULL));

    return pcmIndex > 0;
}

void DSD1024Decoder::extractDSDMetadata(std::ifstream& file, AudioMetadata& metadata) {
    file.seekg(0, std::ios::end);
    size_t fileSize = file.tellg();
    file.seekg(0, std::ios::beg);
...
void DSD1024Decoder::extractDSDIFFMetadata(std::ifstream& file, AudioMetadata& metadata) {
...
void DSD1024Decoder::extractDSFMetadata(std::ifstream& file, AudioMetadata& metadata) {
    ...
    Logger::debug("DSF metadata extracted for DSD1024 format");
}
